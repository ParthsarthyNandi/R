...
##  1.What does a streak length of 1 mean, i.e. how many hits and misses are in a streak of 1? What about a streak length of 0?
    ...
    Streak length means how many time he Hit the shot. Streek length 0 means no goal scored.
...
```{r}
    sim_unfair_coin <- sample(outcomes, size = 1000, replace = TRUE, prob = c(0.2, 0.8))
...
##   read more on the sample function in help.  
...
```{r}
     outcomes2 <- c("H", "M")
     sim_basket <- sample(outcomes2, size = 10, replace = TRUE, prob = c(.5, 0.5))
     table(sim_basket)/10
```
...
```
    ...
    In my opinion it is different because the probabilty of Hit and miss can be differnt each shot.
    I won't seed the as it will give the same set of outputs and we will end up simulating the same.scenario
...
```
    ...
    In my opinion yes the simulation streaks are independant outcomes.
